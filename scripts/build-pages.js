...
const site = {
  ...
  lead: "Use signal-like state, async values, and ordered handler pipelines without adopting a full statechart runtime.",
  quickstart: "pnpm add @async/flow\npnpm run pipeline:verify",
  docsSources: [
    "README.md",
    "CHANGELOG.md",
    "docs"
...
const asyncProjects = [
  ["@async/flow", "https://async.github.io/flow/", "Signal state and handlers"],
...
const docs = await collectDocs(site.docsSources);
...
function home(docs) {
  ...
  return `
    <section class="hero">
      ...
      <p class="sublead">${escapeHtml(site.lead)}</p>
      <div class="actions">
...
<head>
  ...
  <style>
    :root{color-scheme:dark;--bg:#15202b;--soft:#192734;--border:#38444d;--border2:#2f3c47;--text:#f7f9f9;--muted:#8b98a5;--body:#cfd9de;--blue:#1d9bf0;--green:#00ba7c;--shadow:0 24px 80px rgba(2,6,23,.32)}
...
<body>
  <div class="container"><div class="page"><header class="topbar"><a class="brand" href="${rootPrefix}index.html"><span class="mark"><span></span><span></span><span></span><span></span></span><span>${escapeHtml(site.title)}</span></a><nav class="nav">${nav}</nav></header><main>${body}</main><footer>Documentation for ${escapeHtml(site.title)}.</footer></div></div>
</body>
</html>
`;
}

async function collectDocs(sources) {
  const docs = [];
  for (const source of sources) {
    try {
      const info = await stat(source);
      if (info.isDirectory()) {
        await walk(source, docs);
      } else if (info.isFile() && source.endsWith(".md")) {
        await addDoc(source, docs);
      }
    } catch {
      // Missing optional docs roots are ignored.
    }
  }
...
async function walk(dir, docs) {
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (entry.isFile() && entry.name.endsWith(".md")) {
      await addDoc(path, docs);
    }
  }
}

async function addDoc(path, docs) {
  const source = await readFile(path, "utf8");
  const rel = relative(process.cwd(), path).replaceAll("\\", "/");
  const href = rel.replace(/\.md$/, ".html");
  docs.push({ path, source: rel, href, title: firstHeading(source) || basename(path, ".md"), order: docOrder(rel) });
}

function docOrder(source) {
  if (source === "README.md") return 0;
  if (source === "CHANGELOG.md") return 1;
  return 10;
}
...
function renderTextLinks(value) {
  return String(value).split(/(\[[^\]]+\]\([^)]+\))/g).map((segment) => {
    ...
    if (match) return `<a href="${escapeHtml(rewriteMarkdownHref(match[2]))}">${escapeHtml(match[1])}</a>`;
...
function rewriteMarkdownHref(href) {
  ...
  if (/\.md#/.test(href)) return href.replace(/\.md#/, ".html#");
  return href;
}
